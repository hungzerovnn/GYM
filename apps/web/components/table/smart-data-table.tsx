...
const AVATAR_COLUMN_KEYS = new Set(["fullName", "customerInfo"]);
...
const getAvatarUrl = (row: Record<string, unknown>) => {
  const candidates = [row.avatarUrl, row.photoUrl, row.imageUrl, row.profileImageUrl];
...
type CompactLine = {
  ...
  text: string;
  tone?: "muted" | "accent" | "danger" | "normal";
};
...
const buildPackageTimeLines = (row: Record<string, unknown>) => {
  const startDate = formatDateValue(getFirstValue(row, ["startDate", "startTrainingDate"]));
  const endDate = formatDateValue(getFirstValue(row, ["endDate", "endTrainingDate"]));
  const daysRemaining = formatNumberValue(getFirstValue(row, ["daysRemaining"]));

  return [
    { icon: <CalendarDays className="h-3 w-3" />, text: startDate ? `${translateText("Bat dau")}: ${startDate}` : "" },
    { icon: <CalendarDays className="h-3 w-3" />, text: endDate ? `${translateText("Het han")}: ${endDate}` : "" },
    { text: daysRemaining ? `${translateText("Con lai")}: ${daysRemaining} ${translateText("ngay")}` : "", tone: "accent" as const },
  ];
};
...
const renderCustomerBundle = (row: Record<string, unknown>, displayValue: string) => {
  ...
  return <CompactInfoCell avatarUrl={getAvatarUrl(row)} lines={buildCustomerLines(row)} title={title} />;
};
        ...
        .map((column) =>
          columnHelper.accessor(column.key, {
            ...
            cell: (info) => {
              ...
              if (AVATAR_COLUMN_KEYS.has(column.key)) {
                const avatarUrl = getAvatarUrl(info.row.original);
                if (avatarUrl) {
                  return <AvatarCell avatarUrl={avatarUrl} label={displayValue} />;
                }
              }
